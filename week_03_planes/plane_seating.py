...
def seat_economy(plane,economy_sold,name):
    ...
    found_seat = False
    while not(found_seat):
        #CHANGE THIS TO GIVE WINDOW PREFERENCE TO PEOPLE WHO NEED it
        r_row = random.randrange(0,rows)
        r_col = random.randrange(0,cols)
        if "disability" in name or "bf" in name:
           if plane[r_row][r_col] == "win":
             plane[r_row][r_col] = name
             found_seat = True
        #NEED TO IMPLEMENT TO FIND A WINDOW SEAT INSTEAD OF RANDOMLY TRYING TO FIND ONE
        else:
          if plane[r_row][r_col] == "win" or plane[r_row][r_col] == "avail":
              plane[r_row][r_col] = name
              found_seat = True
...
def fill_plane(plane,disability,tall,bfMoms):
    """
    Params: plane - a list of lists representing a plane
    ...
    max_family_size = 3
    while total_seats > 1:
        #SEATING PEOPLE BASED ON PREFERENCES FOR TALL, DISABILITY, BREASTFEEDING
        while dis_number > 0:
          plane = purchase_economy_plus(plane, economy_sold, "ep-%d-disability"%ep_number)
          ep_number = ep_number + 1
          total_seats = get_avail_seats(plane,economy_sold)
          dis_number = dis_number - 1
        while tall_number > 0 : #NEED TO HAVE A WAY TO FIGURE OUT LEG ROOM SEATS AND GIVE THEM TO TALL PEOPLE - CANNOT DO NOW
          plane = purchase_economy_plus(plane, economy_sold, "ep-%d-tall"%ep_number)
          ep_number = ep_number + 1
          total_seats = get_avail_seats(plane,economy_sold)
          tall_number = tall_number - 1
        while bf_number > 0 :
          plane = purchase_economy_plus(plane, economy_sold, "ep-%d-bf"%ep_number)
          ep_number = ep_number + 1
          total_seats = get_avail_seats(plane,economy_sold)
          bf_number = bf_number - 1
